...
if ( ! function_exists( 'hyori_post_thumbnail' ) ) {
	function hyori_post_thumbnail($thumbsize = '', $link = '') {
		...
		global $post;
		if ( ! isset($post) || ! is_object($post) || post_password_required() || is_attachment() || ! has_post_thumbnail($post->ID) ) {
            return '';
        }
		
		if (empty($thumbsize)) {
			$options = isset($GLOBALS['hyori_options']) ? $GLOBALS['hyori_options'] : [];
			$thumbsize = !empty($options['blog_item_thumbsize']) ? $options['blog_item_thumbsize'] : 'medium';
		}
		...
		if (preg_match('/^(\d+)x(\d+)$/', $thumbsize, $matches)) {
			$width = (int) $matches[1];
			$height = (int) $matches[2];
			$image = wp_get_attachment_image_src($product_thumbnail_id, array($width, $height));
			$image_tag = '<img src="' . esc_url($image[0]) . '" width="' . esc_attr($width) . '" height="' . esc_attr($height) . '" alt="' . esc_attr(get_the_title($post)) . '" />';
		} else {
			$image_tag = wp_get_attachment_image($product_thumbnail_id, $thumbsize);
		}
