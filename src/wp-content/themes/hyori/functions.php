...
define( 'HYORI_THEME_VERSION', '1.4.1' );
...
function hyori_enqueue_styles() {
	...
	// magnific-popup
	wp_enqueue_style( 'magnific-popup', get_template_directory_uri() . '/css/magnific-popup.css', array(), '1.4.1' );
	// perfect scrollbar
...
function hyori_enqueue_scripts() {
	...
	// popup
	wp_enqueue_script( 'jquery-magnific-popup', get_template_directory_uri() . '/js/jquery.magnific-popup.min.js', array( 'jquery' ), '1.4.1', true );
	// unviel
	wp_enqueue_script( 'jquery-unveil', get_template_directory_uri() . '/js/jquery.unveil.js', array( 'jquery' ), '1.4.1', true );
	// perfect scrollbar
...
if( hyori_is_goal_framework_activated() ) {
	...
	require get_template_directory() . '/inc/widgets/woo-price-filter-slider.php';
...
add_action( 'wp_enqueue_scripts', 'hyori_enqueue_price_slider_assets', 20 );

// Force WooCommerce slider assets to be loaded when needed
function hyori_enqueue_price_slider_assets() {
    if ( function_exists( 'is_woocommerce' ) && ( is_woocommerce() || is_shop() || is_product_taxonomy() ) ) {
        ...
        if ( function_exists( 'WC' ) ) {
            ...
            wp_localize_script( 'wc-price-slider', 'wc_price_slider_params', array(
                'slider_script_url' => WC()->plugin_url() . '/assets/js/frontend/price-slider.min.js',
            ) );
        }
    }
}

// Clear widget cache when products are updated
add_action( 'save_post_product', 'hyori_clear_price_filter_cache', 3 );
add_action( 'deleted_post', 'hyori_clear_price_filter_cache' );
add_action( 'woocommerce_update_product', 'hyori_clear_price_filter_cache' );

function hyori_clear_price_filter_cache( $post_id = null ) {
    global $wpdb;

    $transients = $wpdb->get_col(
        "SELECT option_name FROM {$wpdb->options}
         WHERE option_name LIKE '_transient_hyori_price_filter_widget_%'"
    );
